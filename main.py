...
def boten(message):
    ...
    M = types.InlineKeyboardButton(' ‹  ♰ مطور البوت ♰  ›', url='https://t.me/J_M_G')
...
def masg(call):
	...
	if call.data =="N_B_1":
		...
		while True:
			...
			un = str(''.join(random.choice(xn)for i in range(1)))
			ua = str(''.join(random.choice(xa)for i in range(1)))
			uh = str(''.join(random.choice(xh)for i in range(1)))
			ud = str(''.join(random.choice(xd)for i in range(1)))
			0h = str(us)+"_"+str(ud)+"_"+str(ua)
			1h = str(us)+"_"+str(un)+"_"+str(ua)
			2h = str(us)+"_"+str(ua)+"_"+str(un)
			3h = str(us)+"_"+str(un)+"_"+str(uh)
			f = [h0,h1,h2,h3]
			j = random.choice(f)
			url = "https://t.me/"+j
			headers = {
...
if __name__ == "__main__":
    bot.remove_webhook()
    bot.set_webhook(url="https://tttt8.herokuapp.com/"+str(BOT_TOKEN))
